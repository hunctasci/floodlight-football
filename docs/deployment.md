...
Public canonical URLs, result shares and production friend invites use `https://hncleague.com`. Homepage metadata includes Open Graph, X cards and VideoGame JSON-LD. The 1200×630 social image is `public/social/country-clash-v2.png`; its editable SVG source is alongside it. `/how-to-play.html` provides crawlable rules and controls. `/sitemap.xml` lists the two public pages, and `/robots.txt` points to it. Search Console sitemap submission requires a verified property; deployment alone does not submit or guarantee indexing.
