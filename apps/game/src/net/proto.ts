import { INPUT_BYTES } from './codec';

/**
 * Match packet framing over a DataTransport (1-byte tag, big-endian u32s).
 * Tag 0 input:    [0, tick u32, INPUT_BYTES input bytes]
...
export function encodeInputPacket(tick: number, bytes: Uint8Array): Uint8Array {
  const out = new Uint8Array(1 + 4 + INPUT_BYTES);
  const v = new DataView(out.buffer);
  out[0] = 0; v.setUint32(1, tick >>> 0); out.set(bytes.subarray(0, INPUT_BYTES), 5);
  return out;
}
...
export function decodePacket(data: Uint8Array): NetPacket {
  try {
    ...
    const tag = data[0];
    if (tag === 0 && data.length >= 5 + INPUT_BYTES) {
      return { kind: 'input', tick: v.getUint32(1), bytes: data.slice(5, 5 + INPUT_BYTES) };
    }
