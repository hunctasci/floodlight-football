...
import { makeClientId, encodeCode } from './net/signal';
import {
...
// Serverless invite-link flow: host holds the pending offer transport, the
// link carries the SDP; the joiner holds the reply code to send back.
let inviteLink = '', inviteReply = '', invitePrefill = '';
let inviteOffer: RTCTransport | null = null;
...
function closeNet() {
  ...
  if (inviteOffer) { try { inviteOffer.close(); } catch { /* already gone */ } inviteOffer = null; }
  if (sig) { try { sig.close(); } catch { /* already gone */ } sig = null; }
  roomCode = ''; matchToken = ''; peerId = '';
  inviteLink = ''; inviteReply = ''; invitePrefill = '';
...
function menu(){ if(!menuDirty)return; menuDirty=false;
  ...
  if(screen==='online') { const items=['CREATE INVITE LINK','CREATE ROOM','JOIN ROOM','BACK']; panel(`<div class="eyebrow">PLAY ONLINE · P2P LOCKSTEP</div><div class="title" style="font-size:38px">ONLINE</div><div class="subtitle">${TEAMS[teamIndex].short} · ${duration/60} MIN HALVES</div>${items.map((x,i)=>`<div class="menu-item ${menuIndex===i?'selected':''}" data-mi="${i}">${menuIndex===i?'▶ ':''}${x}</div>`).join('')}<div class="hint">INVITE LINK WORKS WITH NO SERVER · ROOM CODES NEED A LEAGUE SERVER<br>↑ / ↓ SELECT · ENTER CONFIRM · ESC BACK</div>`); return; }
  if(screen==='invitehost') { panel(`<div class="eyebrow">SERVERLESS 2-PLAYER · YOU ARE TEAM 1</div><div class="title" style="font-size:38px">INVITE</div><div class="subtitle">${netStatus || '…'}</div>${inviteLink?`<textarea class="netpaste scorebox netcode" id="invitelink" rows="2" readonly>${inviteLink}</textarea><div class="hint">TAP TO COPY · SEND IT IN ANY CHAT APP</div><textarea class="netpaste scorebox" id="inviteanswer" rows="1" placeholder="PASTE THE REPLY CODE"></textarea><div class="menu-item netbtn" data-act="invite-answer">▶ CONNECT</div>`:''}<div class="menu-item netbtn" data-act="cancel">▶ CANCEL</div>`); return; }
  if(screen==='invitejoin') { panel(`<div class="eyebrow">SERVERLESS 2-PLAYER · YOU ARE TEAM 2</div><div class="title" style="font-size:38px">JOIN</div><div class="subtitle">${netStatus || 'PASTE THE INVITE LINK'}</div>${!inviteReply&&!net?`<textarea class="netpaste scorebox" id="invitecode" rows="2" placeholder="PASTE THE INVITE LINK OR CODE">${invitePrefill}</textarea><div class="menu-item netbtn" data-act="invite-open">▶ OPEN INVITE</div>`:''}${inviteReply?`<textarea class="netpaste scorebox netcode" id="invitereply" rows="2" readonly>${inviteReply}</textarea><div class="hint">TAP TO COPY · SEND THIS REPLY BACK — THE MATCH CONNECTS ITSELF</div>`:''}<div class="menu-item netbtn" data-act="cancel">▶ CANCEL</div>`); return; }
...
// --- Serverless invite links: WebRTC handshake carried by copy-paste, no server at all. ---
function startInviteHost() {
  if (netBusy) return; netBusy = true;
  const token = ++netGen;
  const alive = () => token === netGen && screen === 'invitehost';
  netStatus = 'CREATING INVITE…'; menuDirty = true;
  void RTCTransport.createOfferSdp().then(({ transport, offer }) => {
    if (!alive()) { transport.close(); return; }
    inviteOffer = transport;
    inviteLink = `${location.origin}${location.pathname}?invite=${encodeCode({ sdp: offer })}`;
    netStatus = 'SEND THIS LINK TO YOUR FRIEND'; menuDirty = true;
  }).catch(() => { if (alive()) deadNet('WEBRTC UNAVAILABLE HERE'); });
}
function doInviteAnswer() {
  const code = areaVal('inviteanswer');
  if (!code || !inviteOffer) return;
  const offer = inviteOffer; inviteOffer = null;
  netStatus = 'CONNECTING…'; menuDirty = true;
  offer.acceptAnswer(code).then(() => hookDriver(new NetDriver(offer, { host: true, teamIndex, duration })))
    .catch(() => { if (screen === 'invitehost') deadNet('BAD REPLY CODE'); });
}
function startInviteJoin(raw: string) {
  if (netBusy) return; netBusy = true;
  const token = ++netGen;
  const alive = () => token === netGen && screen === 'invitejoin';
  netStatus = 'OPENING INVITE…'; menuDirty = true;
  const m = raw.match(/invite=([A-Za-z0-9\-_]+)/);
  const code = m ? m[1] : raw.trim();
  void RTCTransport.acceptOffer(code).then(({ transport, answer }) => {
    if (!alive()) { transport.close(); return; }
    inviteReply = answer;
    netStatus = 'SEND THE REPLY BACK'; menuDirty = true;
    hookDriver(new NetDriver(transport, { host: false }));
  }).catch(() => { if (alive()) deadNet('BAD INVITE LINK'); });
}
function doReady() {
...
function handleMenuEnter(act?: string) {
  if (screen === 'title') {
    ...
    if (menuIndex === 0) { screen = 'invitehost'; menuIndex = 0; netStatus = ''; inviteLink = ''; inviteReply = ''; startInviteHost(); }
    else if (menuIndex === 1) { screen = 'netcreate'; menuIndex = 0; roomCode = ''; netStatus = ''; startHost(); }
    else if (menuIndex === 2) { screen = 'netjoin'; menuIndex = 0; roomCode = ''; netStatus = ''; }
    else screen = 'title';
  }
  else if (screen === 'invitehost') {
    if (act === 'cancel') cancelNet();
    else if (act === 'invite-answer') doInviteAnswer();
  }
  else if (screen === 'invitejoin') {
    if (act === 'cancel') cancelNet();
    else if (act === 'invite-open') { const raw = areaVal('invitecode'); if (raw) startInviteJoin(raw); else { netStatus = 'PASTE THE INVITE LINK FIRST'; menuDirty = true; } }
  }
...
// Invite links (?invite=…): land the joiner straight on the JOIN screen.
if (NET_LIVE) {
  const inviteParam = new URLSearchParams(location.search).get('invite');
  if (inviteParam) { screen = 'invitejoin'; invitePrefill = inviteParam; }
}
