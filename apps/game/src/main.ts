...
function launch(){closeNet();dailyMode=false;engine=new MatchEngine(teamIndex,flowTest?8:duration,Math.floor(Math.random()*999999),aiLevel);viewTeam=0;renderer.setFollow(null,null);screen='match';menuIndex=0;audio.event({type:'whistle'});}
/** Daily Cup: one deterministic match per calendar day, same seed worldwide. */
function launchDaily(){closeNet();dailyMode=true;engine=new MatchEngine(teamIndex,flowTest?8:duration,dailySeed(),aiLevel);viewTeam=0;renderer.setFollow(null,null);screen='match';menuIndex=0;audio.event({type:'whistle'});}
