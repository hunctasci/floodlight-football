...
test('codec round-trips buttons, axes and aim in 6 bytes', () => {
  const f: InputFrame = {
  ...
  for (const k of ['sprint', 'pass', 'passHeld', 'shootPressed', 'shootHeld', 'switchPlayer', 'long'] as const) assert.ok(d[k]);
