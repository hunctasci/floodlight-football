...
import { createApp, type AppHandles } from '../src/server.ts';
...
class Client {
  ...
  constructor(readonly port: number) {
    ...
    this.ws.on('message', (d) => {
      ...
      if (w) w(s); else this.queue.push(s);
    });
  }
  ...
  next(timeoutMs = 1500): Promise<Record<string, unknown>> {
...
test('healthz reports shape and unknown routes 404', async (t) => {
  ...
  t.after(() => app.close());
  const h = await (await fetch(`http://127.0.0.1:${port}/healthz`)).json() as Record<string, unknown>;
...
test('create, join, signal relay, leave lifecycle', async (t) => {
  ...
  const created = await a.next() as { t: string; roomCode: string; matchToken: string };
  ...
  const pj = await a.next() as { t: string; clientId: string };
...
test('join of missing/full rooms is rejected; empty rooms vanish', async (t) => {
  ...
  a.send({ t: 'create-room', clientId: A });
  const { roomCode } = await a.next() as { roomCode: string };
  ...
  await b.next(); await a.next(); // joined + peer-joined
...
test('create rate limit trips and resets', async (t) => {
  ...
  for (const id of ids.slice(0, 2)) { a.send({ t: 'create-room', clientId: id }); await a.next(); }
