...
test('manager allocates, joins idempotently, caps at two, normalizes case', async () => {
  ...
  await assert.rejects(m.join('ABCDEA', 'CCCCCCC3'), (e: unknown) => e instanceof RoomError && e.code === 'FULL');
  await assert.rejects(m.join('ZZZZZZ', 'CCCCCCC3'), (e: unknown) => e instanceof RoomError && e.code === 'NOT_FOUND');
});
...
test('manager retries code collisions then gives up', async () => {
  ...
  await assert.rejects(m.create('BBBBBBB2'), (e: unknown) => e instanceof RoomError && e.code === 'EXISTS');
});
