...
test('deterministic across instances; snapshot/restore resumes identically', () => {
  const drive = (frame: number) => ({
    ...
    peer: { x: -1, z: frame % 90 < 45 ? -0.2 : 0.2, sprint: frame % 4 === 0, through: frame % 230 === 100 } as Partial<InputFrame>,
  });
  const run = (frames: number, from?: MatchEngine) => {
    ...
    for (let f = 0; f < frames; f++) {
      ...
      const kick = r ? { pass: r.team === 0, through: false, cross: r.team === 0 && g.state.phase === 'corner', shootPressed: false, x: 1, z: 0 } : {};
      const pkick = r ? { pass: r.team === 1, through: false, cross: false, shootPressed: r.team === 1 && g.state.phase === 'goalkick', x: -1, z: 0 } : {};
