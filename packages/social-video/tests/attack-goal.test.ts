...
test('beat 2 (frame 30): ball travelling between passer and receiver', () => {
  const compiled = std();
  const desc = goalDesc(compiled, 30);
  const release = goalDesc(compiled, 21).ball; // pass release moment (t=0.7)
  const recv = actor(goalDesc(compiled, 45), 'Winger'); // catch moment (t=1.5)
...
test('beat 4 (frame 90): ball at the shooter\'s feet, keeper set near goal', () => {
  const desc = goalDesc(std(), 90);
...
test('beat 5 (frame 105): ball flying at goal, keeper committed to the dive', () => {
  const desc = goalDesc(std(), 105);
...
test('beat 7 (frame 115): ball inside the goal', () => {
  const desc = goalDesc(std(), 115);
...
test('beat 9 (frame 150): shooter celebration is active', () => {
  const desc = goalDesc(std(), 150); // seed 42 → variant 0, arms-up V
...
test('continuity: ball never jumps outside the shot window', () => {
  const compiled = std();
  for (let f = 0; f < 179; f++) {
    if (f >= 94 && f < 111) continue; // the driven shot is meant to be fast
...
test('the shot itself is fast: over 1m per frame mid-flight', () => {
  const compiled = std();
  const a = goalDesc(compiled, 100).ball, b = goalDesc(compiled, 101).ball;
...
test('determinism: same spec, separately compiled, identical at every beat', () => {
  const a = std(), b = std();
  for (const frame of [0, 30, 60, 90, 105, 115, 150, 179]) {
    assert.deepEqual(evaluateFrame(a, frame), evaluateFrame(b, frame));
  }
});

test('random-access: frame 115 is independent of evaluation order', () => {
  const compiled = std();
  const direct = evaluateFrame(compiled, 115);
  for (const f of [0, 10, 60, 90, 150, 179, 40, 20]) evaluateFrame(compiled, f);
  assert.deepEqual(evaluateFrame(compiled, 115), direct);
});
...
test('seed variation: 42 vs 43 differ in style, agree on meaning', () => {
  ...
  const armsUp = actor(goalDesc(a, 150), 'Shooter');
  assert.ok(armsUp.armLift > 1.5 && armsUp.armSpread > 0.5, 'seed 42 celebrates arms-up');
  assert.ok(Math.abs(actor(goalDesc(b, 150), 'Shooter').spin) > 0.5, 'seed 43 celebrates with a spin');
});
...
test('away attack mirrors the choreography onto the far goal', () => {
  ...
  const desc = goalDesc(compiled, 115);
...
test('actors stay on the pitch with normalized facing', () => {
  const compiled = std();
  for (const frame of [0, 60, 105, 150]) {
...
test('camera cuts between beats but flows within them', () => {
  ...
  assert.ok(jump(77, 79) > 2, 'hard cut into the shooter closeup at 2.6s');
  assert.ok(jump(60, 61) < 0.5, 'smooth tracking inside the attack beat');
  for (const frame of [0, 105, 150, 179]) {
...
test('out-of-range and foreign-scene frames fail clearly', () => {
  const compiled = std();
  assert.throws(() => evaluateFrame(compiled, 180), /Invalid frame: 180/);
