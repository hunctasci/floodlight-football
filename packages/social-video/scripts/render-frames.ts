...
import { renderFramesToDir, renderTemplateFramesToDir } from '../src/render/frame';
import { compileTemplate } from '../src/templates/compile';
import { outputFromArgs, readArgs, specInputFromArgs } from './cli-args';

function usage(): string {
  return [
    'Usage: social:frames --home <CODE> --away <CODE> [--scene faceoff|attack-goal] [--template country-rivalry-reel] [--format reel] [--seed 42] [--fps 30] [--duration <sec>] [--attack-team home|away] [--attack-style central|wing|counter] [--headline <text>] [--secondary <text>] [--cta <text>] [--no-overlays] --output <dir/>',
    '',
    'Examples:',
